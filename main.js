...
async function connectWallet() {
  if (window.ethereum) {
    try {
      ...
      alert("⚠️ ไม่สามารถเชื่อมต่อกระเป๋า: " + err.message);
    }
  } else {
...
async function buyAndStake() {
  ...
  const amountOutMin = web3.utils.toWei(minKJC, "ether");   // KJC 18 decimals
...
async function loadUserInfo() {
  try {
    ...
    document.getElementById("yourStake").innerText =
      "ยอด Stake: " + web3.utils.fromWei(stake, "ether") + " KJC";
    document.getElementById("yourReward").innerText =
      "รางวัลแนะนำ: " + web3.utils.fromWei(reward, "ether") + " KJC";
  } catch (e) {
